...
class FoodDetailView: UIView {
    ...
    private let addFoodButton = UIButton(type: .system)
    ...
    private func setupAddButton() {
        addFoodButton.setTitle("TRACK", for: .normal)
        addFoodButton.titleLabel?.font = UIFont.boldSystemFont(ofSize: 16)
        addFoodButton.backgroundColor = UIColor.systemGreen
        addFoodButton.setTitleColor(.white, for: .normal)
        addFoodButton.layer.cornerRadius = 6
        addFoodButton.addTarget(self, action: #selector(addMealButtonTapped), for: .touchUpInside)
        
        addFoodButton.translatesAutoresizingMaskIntoConstraints = false
        addSubview(addFoodButton)
        
        NSLayoutConstraint.activate([
            addFoodButton.topAnchor.constraint(equalTo: nutritionSummaryView.bottomAnchor, constant: 40),
            addFoodButton.leadingAnchor.constraint(equalTo: safeAreaLayoutGuide.leadingAnchor, constant: 20),
            addFoodButton.trailingAnchor.constraint(equalTo: safeAreaLayoutGuide.trailingAnchor, constant: -20)
        ])
    }
